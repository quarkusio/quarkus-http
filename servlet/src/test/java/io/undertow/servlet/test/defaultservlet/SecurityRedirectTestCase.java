...
public class SecurityRedirectTestCase {
    ...
    public void testSecurityWithWelcomeFileRedirect() throws IOException {
        ...
        try {
            ...
            Assert.assertEquals(HttpResponseStatus.FOUND.code(), result.getStatusLine().getStatusCode());
            Header[] values = result.getHeaders(HttpHeaderNames.LOCATION);
            ...
            Assert.assertEquals(HttpResponseStatus.UNAUTHORIZED.code(), result.getStatusLine().getStatusCode());

            values = result.getHeaders(HttpHeaderNames.WWW_AUTHENTICATE);
            assertEquals(1, values.length);
            assertEquals(HttpHeaderNames.BASIC + " realm=\"Test Realm\"", values[0].getValue());
            HttpClientUtils.readResponse(result);

            get = new HttpGet(DefaultServer.getDefaultServerURL() + "/servletContext/");
            get.addHeader(HttpHeaderNames.AUTHORIZATION, HttpHeaderNames.BASIC + " " + FlexBase64.encodeString("user1:password1".getBytes(), false));
            result = client.execute(get);
            String response = HttpClientUtils.readResponse(result);
            Assert.assertEquals(HttpResponseStatus.OK.code(), result.getStatusLine().getStatusCode());
    ...
    public void testSecurityWithoutWelcomeFileRedirect() throws IOException {
        ...
        try {
            ...
            Header[] values = result.getHeaders(HttpHeaderNames.WWW_AUTHENTICATE);
            assertEquals(1, values.length);
            assertEquals(HttpHeaderNames.BASIC + " realm=\"Test Realm\"", values[0].getValue());
            HttpClientUtils.readResponse(result);
            ...
            result = client.execute(get);
            Assert.assertEquals(HttpResponseStatus.FOUND.code(), result.getStatusLine().getStatusCode());
            values = result.getHeaders(HttpHeaderNames.LOCATION);
            assertEquals(1, values.length);
            ...
            String response = HttpClientUtils.readResponse(result);
            Assert.assertEquals(HttpResponseStatus.OK.code(), result.getStatusLine().getStatusCode());
            Assert.assertEquals("Stuart", response);
        } finally {
            client.getConnectionManager().shutdown();
        }
    }

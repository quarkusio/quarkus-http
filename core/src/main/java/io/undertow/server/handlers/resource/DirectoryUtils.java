...
public class DirectoryUtils {
    ...
    public static class Blobs {
        ...
        public static final ByteBuf FILE_CSS_BUFFER;
        public static final ByteBuf FILE_JS_BUFFER;

        static {
            try {
                byte[] bytes = FILE_CSS.getBytes(StandardCharsets.US_ASCII);
                FILE_CSS_BUFFER = Unpooled.buffer(bytes.length);
                FILE_CSS_BUFFER.writeBytes(bytes);

                bytes = FILE_JS.getBytes(StandardCharsets.US_ASCII);
                FILE_JS_BUFFER = Unpooled.buffer(bytes.length);
                FILE_JS_BUFFER.writeBytes(bytes);
            } catch (Exception e) {

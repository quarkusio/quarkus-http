...
public interface JsrWebSocketMessages {
    ...
    RuntimeException endpointDoesNotHaveAppropriateConstructor(Class<?> endpoint);
    ...
    @Message(id = 3043, value = "Path must be specified")
    IllegalArgumentException pathMustBeSpecified();

    @Message(id = 3044, value = "Could not path URI template %s at position %s")
    RuntimeException couldNotParseUriTemplate(String path, int length);

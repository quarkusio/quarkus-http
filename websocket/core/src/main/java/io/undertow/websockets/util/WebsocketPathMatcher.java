 ...
 */
public class WebsocketPathMatcher<T> {

    /**
     * Map of path template stem to the path templates that share the same base.
     */
    private Map<String, Set<PathTemplateHolder>> pathTemplateMap = new ConcurrentHashMap<>();
    ...
    public synchronized WebsocketPathMatcher<T> add(final PathTemplate template, final T value) {
        ...
        if (newValues.contains(holder)) {
            ...
            throw JsrWebSocketMessages.MESSAGES.matcherAlreadyContainsTemplate(template.getTemplateString(), equivalent.getTemplateString());
        }
    ...
    public synchronized WebsocketPathMatcher<T> add(final String pathTemplate, final T value) {
        final PathTemplate template = PathTemplate.create(pathTemplate);
        return add(template, value);
    }

    public synchronized WebsocketPathMatcher<T> addAll(WebsocketPathMatcher<T> pathTemplateMatcher) {
    ...
    public synchronized WebsocketPathMatcher<T> remove(final String pathTemplate) {
        final PathTemplate template = PathTemplate.create(pathTemplate);
        return remove(template);
    }

    private synchronized WebsocketPathMatcher<T> remove(PathTemplate template) {
        Set<PathTemplateHolder> values = pathTemplateMap.get(trimBase(template));
        Set<PathTemplateHolder> newValues;
        if (values == null) {
            return this;

...
public class Encoding implements Closeable {
    ...
    public Encoding(final Map<Class<?>, List<ObjectHandle<? extends Encoder>>> binaryEncoders, final Map<Class<?>, List<ObjectHandle<? extends Decoder>>> binaryDecoders, final Map<Class<?>, List<ObjectHandle<? extends Encoder>>> textEncoders, final Map<Class<?>, List<ObjectHandle<? extends Decoder>>> textDecoders) {
    ...
    public Object decodeText(final Class<?> targetType, final String message) throws DecodeException {
        ...
        List<ObjectHandle<? extends Decoder>> decoders = textDecoders.get(targetType);
        if (decoders != null) {
    ...
    public Object decodeBinary(final Class<?> targetType, final byte[] bytes) throws DecodeException {
        List<ObjectHandle<? extends Decoder>> decoders = binaryDecoders.get(targetType);
        if (decoders != null) {
            for (ObjectHandle<? extends Decoder> decoderHandle : decoders) {
                Decoder decoder = decoderHandle.getInstance();
    ...
    public String encodeText(final Object o) throws EncodeException {
        List<ObjectHandle<? extends Encoder>> encoders = textEncoders.get(o.getClass());
        if(encoders == null) {
            for(Map.Entry<Class<?>, List<ObjectHandle<? extends Encoder>>> entry : textEncoders.entrySet()) {
        ...
        if (encoders != null) {
            for (ObjectHandle<? extends Encoder> decoderHandle : encoders) {
    ...
    public ByteBuffer encodeBinary(final Object o) throws EncodeException {
        List<ObjectHandle<? extends Encoder>> encoders = binaryEncoders.get(o.getClass());

        if(encoders == null) {
            for(Map.Entry<Class<?>, List<ObjectHandle<? extends Encoder>>> entry : binaryEncoders.entrySet()) {
                if(entry.getKey().isAssignableFrom(o.getClass())) {
                    encoders = entry.getValue();
                    break;
                }
    ...
    @Override
    public void close() {
        for (Map.Entry<Class<?>, List<ObjectHandle<? extends Decoder>>> entry : binaryDecoders.entrySet()) {
            for (ObjectHandle<? extends Decoder> val : entry.getValue()) {
                val.getInstance().destroy();
                val.release();
            }
        }
        for (Map.Entry<Class<?>, List<ObjectHandle<? extends Decoder>>> entry : textDecoders.entrySet()) {
            for (ObjectHandle<? extends Decoder> val : entry.getValue()) {
                val.getInstance().destroy();
                val.release();
            }
        ...
        for (Map.Entry<Class<?>, List<ObjectHandle<? extends Encoder>>> entry : binaryEncoders.entrySet()) {
            for (ObjectHandle<? extends Encoder> val : entry.getValue()) {
                val.getInstance().destroy();
                val.release();
            }
        }
